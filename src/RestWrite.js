...
RestWrite.prototype.handleAuthData = async function (authData) {
  ...
  if (isLogin || isCurrentUserLoggedOrMaster) {
    ...
    for (let key of Object.keys(withoutUnlinked)) {
      if (withoutUnlinked[key] && withoutUnlinked[key].id) {
        authDataResponse[key] = {
          id: withoutUnlinked[key].id,
        };
      }
    }

    if (Object.keys(authDataResponse).length) {
      this.authDataResponse = authDataResponse;
    }
    ...
    if (!this.authDataResponse && !!Object.keys(withoutUnlinked).length) {
      this.authDataResponse = withoutUnlinked;
    }
    ...
    if (hasMutatedAuthData || !this.config.allowExpiredAuthDataToken) {
      ...
      if (hasUnlink) {
        for (let key of Object.keys(unlink)) {
