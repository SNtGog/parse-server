describe('AuthData Delta Behavior', () => {
  ...
  describe('Provider Linking', () => {
    it('should link Twitter without affecting unchanged Google Play Games auth', async () => {
      ...
      user.set('authData', {
        ...currentAuthData,
        twitter: { id: 'T1', access_token: 'token123' },
      });
      ...
      expect(finalAuthData.twitter?.id).toBe('T1');
    });
    ...
    it('should handle multiple providers correctly', async () => {
      ...
      const authData = {
        ...
        facebook: { id: 'F1', access_token: 'fb_token' },
      };
      ...
      user.set('authData', {
        facebook: currentAuthData.facebook,
        twitter: { id: 'T2', access_token: 'tw_token' },
      ...
      expect(finalAuthData.facebook?.id).toBe('F1');
      expect(finalAuthData.twitter?.id).toBe('T2');
    });
  });
  ...
  describe('Data Validation Optimization', () => {
    ...
    it('should handle empty authData gracefully', async () => {
      ...
      user.set('authData', {
        twitter: { id: 'T3', access_token: 'token456' },
      });
      await user.save(null, { sessionToken });
      ...
      expect(finalAuthData.twitter?.id).toBe('T3');
    });
  });
  ...
  describe('API Call Optimization', () => {
    ...
    it('should handle mixed authData operations without redundant API calls', async () => {
      ...
      freshUser.set('authData', {
        ...currentAuthData,
        facebook: { id: 'fb123', access_token: 'fb_token' }
      });
      await freshUser.save(null, { sessionToken });
      ...
      expect(finalAuthData.gpgames?.id).toBe(MOCK_USER_ID);
      expect(finalAuthData.facebook?.id).toBe('fb123');
    });
  });
});
